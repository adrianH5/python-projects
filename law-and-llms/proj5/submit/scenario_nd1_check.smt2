; ===========================================================
; File: scenario_nd1_check.smt2
; Check for Scenario ND1 (Emily - Non-Dependent)
; ===========================================================
...
; --- Scenario ND1 Specific Declarations and Facts ---
(declare-const Ben Person)
(declare-const Emily Person)
(assert (! (= TP Ben) :named fact_tp_is_ben))
(assert (! (= PD Emily) :named fact_pd_is_emily))

(assert (! (= (Age Ben) 40) :named fact_ben_age))
(assert (! (= (Age Emily) 16) :named fact_emily_age))
(assert (! (IsUSCitizenOrNational Ben) :named fact_citizen_ben))
(assert (! (IsUSCitizenOrNational Emily) :named fact_citizen_emily))
(assert (! (IsCitizenOrNationalOrResident Emily) :named fact_residency_emily))

(assert (! (IsChildOfSibling Emily Ben) true) :named fact_emily_is_niece)) ; Relationship for QR

; Residency
(assert (! (ResidesWithTPMoreThanHalfYear Ben Emily) false) :named fact_emily_lived_with_parents)) ; Key QC failure

; Support
(assert (! (TPProvidesOverHalfSupport Ben Emily) false) :named fact_ben_provides_30_support)) ; QR support failure likely
; Need to assert PD didn't provide own support for QC/QR path check
(assert (! (= (PDProvidesOverHalfOwnSupport Emily) false) :named fact_emily_not_own_support)) ; Assume parents provide > 50%

; Income
(assert (! (= (GrossIncome Emily) 6000) :named fact_emily_income_high)) ; Key QR failure

; Other
(assert (! (= (IsPermanentlyTotallyDisabled Emily) false) :named fact_emily_not_disabled))
(assert (! (= (IsStudent Emily) true) :named fact_emily_is_student)) ; Assume high school student
(assert (! (= (FilesJointReturn Emily) false) :named fact_emily_not_joint_return))
(assert (! (= (IsDependentOfAnyTaxpayer Emily) true) :named fact_emily_dep_of_parents)) ; Likely dependent of parents
(assert (! (= (IsQC_OfAnyOtherTaxpayer Emily) true) :named fact_emily_qc_of_parents)) ; Likely QC of parents
(assert (! (= (HasMultipleSupportAgreement Ben Emily) false) :named fact_no_msa_nd1))

; Set income threshold
(assert (! (= ExemptionAmountThreshold 4700) :named fact_threshold_set_nd1)) ; Emily's income is higher

; --- Goal Assertion for Non-Dependent Scenario ---
(assert (! (IsDependent Ben Emily) :named goal_is_dependent))
