; ===========================================================
; File: scenario_d1_check.smt2
; Check for Scenario D1 (Leo - Dependent)
; ===========================================================
...
; --- Scenario D1 Specific Declarations and Facts ---
(declare-const Maria Person)
(declare-const Leo Person)
(assert (! (= TP Maria) :named fact_tp_is_maria))
(assert (! (= PD Leo) :named fact_pd_is_leo))

(assert (! (= (Age Maria) 45) :named fact_maria_age))
(assert (! (= (Age Leo) 22) :named fact_leo_age))
(assert (! (IsUSCitizenOrNational Maria) :named fact_citizen_maria))
(assert (! (IsUSCitizenOrNational Leo) :named fact_citizen_leo))
(assert (! (IsCitizenOrNationalOrResident Leo) :named fact_residency_leo)) ; Assume US citizen meets this

(assert (! (IsChildOrDescendant Leo Maria) :named fact_leo_is_son))
(assert (! (IsStudent Leo) :named fact_leo_is_student))
(assert (! (ResidesWithTPMoreThanHalfYear Maria Leo) :named fact_leo_residency_dorm_breaks)) ; Key residency fact
(assert (! (TPProvidesOverHalfSupport Maria Leo) :named fact_maria_provides_70_support)) ; Key support fact
(assert (! (= (GrossIncome Leo) 5000) :named fact_leo_income_5k))
(assert (! (= (PDProvidesOverHalfOwnSupport Leo) false) :named fact_leo_not_own_support)) ; Key support fact
(assert (! (= (FilesJointReturn Leo) false) :named fact_leo_not_joint_return))
(assert (! (= (FilesJointReturnOnlyForRefund Leo) false) :named fact_leo_not_joint_return_only_refund))

; Assume Leo is not disabled and not claimed by others
(assert (! (= (IsPermanentlyTotallyDisabled Leo) false) :named fact_leo_not_disabled))
(assert (! (= (IsDependentOfAnyTaxpayer Leo) false) :named fact_leo_not_dep_of_other))
(assert (! (= (IsQC_OfAnyOtherTaxpayer Leo) false) :named fact_leo_not_qc_of_other))
(assert (! (= (HasMultipleSupportAgreement Maria Leo) false) :named fact_no_msa_d1))

; Set income threshold for QR tests (even if not primary path)
(assert (! (= ExemptionAmountThreshold 4700) :named fact_threshold_set_d1))

; --- Goal Assertion for Dependent Scenario ---
(assert (! (not (IsDependent Maria Leo)) :named goal_is_not_dependent))
