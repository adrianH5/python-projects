; ===========================================================
; File: scenario_d3_check.smt2
; Check for Scenario D3 (George - Dependent)
; ===========================================================
...
; --- Scenario D3 Specific Declarations and Facts ---
(declare-const Chloe Person)
(declare-const George Person)
(assert (! (= TP Chloe) :named fact_tp_is_chloe))
(assert (! (= PD George) :named fact_pd_is_george))

(assert (! (= (Age Chloe) 35) :named fact_chloe_age))
(assert (! (= (Age George) 70) :named fact_george_age))
(assert (! (IsUSCitizenOrNational Chloe) :named fact_citizen_chloe))
(assert (! (IsUSCitizenOrNational George) :named fact_citizen_george))
(assert (! (IsCitizenOrNationalOrResident George) :named fact_residency_george))

(assert (! (IsParentOrAncestor George Chloe) true) :named fact_george_is_father)) ; Key relationship
(assert (! (ResidesWithTPMoreThanHalfYear Chloe George) false) :named fact_george_lives_separately))
(assert (! (HasSamePrincipalPlaceOfAbode George Chloe) false) :named fact_george_lives_separately_abode))

(assert (! (= (GrossIncome George) 4500) :named fact_george_income_low)) ; Key income
(assert (! (TPProvidesOverHalfSupport Chloe George) true) :named fact_chloe_provides_over_half_support)) ; Key support
(assert (! (= (PDProvidesOverHalfOwnSupport George) false) :named fact_george_not_own_support))

(assert (! (= (IsPermanentlyTotallyDisabled George) false) :named fact_george_not_disabled))
(assert (! (= (IsQualifyingChild Chloe George) false) :named fact_george_not_qc_of_chloe))
(assert (! (= (IsQC_OfAnyOtherTaxpayer George) false) :named fact_george_not_qc_of_other))
(assert (! (= (FilesJointReturn George) false) :named fact_george_not_joint_return))
(assert (! (= (IsDependentOfAnyTaxpayer George) false) :named fact_george_not_dep_of_other))
(assert (! (= (HasMultipleSupportAgreement Chloe George) false) :named fact_no_msa_d3))

; Set income threshold
(assert (! (= ExemptionAmountThreshold 4700) :named fact_threshold_set_d3)) ; George's income is less

; --- Goal Assertion for Dependent Scenario ---
(assert (! (not (IsDependent Chloe George)) :named goal_is_not_dependent))
