; ===========================================================
; File: scenario_nd2_check.smt2
; Check for Scenario ND2 (Sam - Non-Dependent)
; ===========================================================
...
; --- Scenario ND2 Specific Declarations and Facts ---
(declare-const Rachel Person)
(declare-const Sam Person)
(assert (! (= TP Rachel) :named fact_tp_is_rachel))
(assert (! (= PD Sam) :named fact_pd_is_sam))

(assert (! (= (Age Rachel) 38) :named fact_rachel_age))
(assert (! (= (Age Sam) 17) :named fact_sam_age))
(assert (! (IsUSCitizenOrNational Rachel) :named fact_citizen_rachel))
(assert (! (IsUSCitizenOrNational Sam) :named fact_citizen_sam))
(assert (! (IsCitizenOrNationalOrResident Sam) :named fact_residency_sam))

(assert (! (IsChildOrDescendant Sam Rachel) true) :named fact_sam_is_son))
(assert (! (ResidesWithTPMoreThanHalfYear Rachel Sam) true) :named fact_sam_lived_with_rachel))

; Support
(assert (! (= (PDProvidesOverHalfOwnSupport Sam) true) :named fact_sam_provides_own_support)) ; Key QC failure
; We still need TP support info for QR path check, assume Rachel provided < 50%
(assert (! (TPProvidesOverHalfSupport Rachel Sam) false) :named fact_rachel_provides_less_half_support))

; Income
(assert (! (= (GrossIncome Sam) 25000) :named fact_sam_income_high)) ; Relevant for QR check

; Other
(assert (! (= (IsPermanentlyTotallyDisabled Sam) false) :named fact_sam_not_disabled))
(assert (! (= (IsStudent Sam) false) :named fact_sam_not_student_ext)) ; Not student for age 24 rule
(assert (! (= (FilesJointReturn Sam) false) :named fact_sam_not_joint_return))
(assert (! (= (IsDependentOfAnyTaxpayer Sam) false) :named fact_sam_not_dep_of_other))
(assert (! (= (IsQC_OfAnyOtherTaxpayer Sam) false) :named fact_sam_not_qc_of_other))
(assert (! (= (HasMultipleSupportAgreement Rachel Sam) false) :named fact_no_msa_nd2))

; Set income threshold
(assert (! (= ExemptionAmountThreshold 4700) :named fact_threshold_set_nd2)) ; Sam's income is higher

; --- Goal Assertion for Non-Dependent Scenario ---
(assert (! (IsDependent Rachel Sam) :named goal_is_dependent))
