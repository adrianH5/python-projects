        ...
        if bool_list[i] == False:
            ...
            if len(_word_list) == 1:
                ...
                label_list = (
                    label_list
                    + [f"B-{bool_list[i]}"]
                    + [f"I-{bool_list[i]}"] * (len(_word_list) - 2)
                    + [f"E-{bool_list[i]}"]
                )
...
with open(
    f"/home/gy237/project/Biomedical_datasets/NER/data_updated/{_type}_data_updated.csv",
    "w",
    newline="",
    encoding="utf-8",
) as file:
