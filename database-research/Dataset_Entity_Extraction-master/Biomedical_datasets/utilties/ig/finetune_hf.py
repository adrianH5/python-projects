    ...
    def prediction_step(
        ...
        **gen_kwargs,
        ...
        loss, generated_tokens, labels = super().prediction_step(
            # model.cuda(), inputs, prediction_loss_only, ignore_keys, **gen_kwargs
            model,
            inputs,
...
def load_tokenizer_and_model(
    ...
    if peft_config is not None:
        ...
        if peft_config.peft_type.name == "LORA":
            model = AutoModelForCausalLM.from_pretrained(
                model_dir, trust_remote_code=True, empty_init=False, use_cache=False
            )
...
def main(
    ...
    trainer = Seq2SeqTrainer(
        ...
        eval_dataset=val_dataset.select(list(range(36))),  # 50
        tokenizer=tokenizer,
