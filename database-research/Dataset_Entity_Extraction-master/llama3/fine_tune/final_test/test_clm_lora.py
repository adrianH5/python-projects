...
class CustomTrainer(Trainer):
    def evaluate(self, eval_dataset=None, **kwargs):
        # 首先调用父类的 evaluate 方法，获取标准评估指标
        eval_results = super().evaluate(eval_dataset, **kwargs)

        # 如果评估数据集存在
        if eval_dataset is not None:
            # 创建一个 DataLoader 来获取批次数据
            ...
            print("eval_dataset is None")
            print(all_outputs)
        print(eval_results)
        return eval_results


# # 使用 CustomTrainer 进行评估
...
# print("Model Outputs:", metrics.get("model_outputs"))
################################################################
...
def main():
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        if not data_args.streaming:
            tokenized_datasets = raw_datasets.map(
                (
                    tokenize_function
                    if train_on_inputs == True
                    else generate_and_tokenize_prompt
                ),
    ...
    path = "output_lr1e-5/checkpoint-2500"
    json_path = (
        f"/home/gy237/project/llama3/fine_tune/final_test/final_test_{path}.json"
    )
    ...
    eval_dataset = trainer.get_eval_dataloader().dataset
    # 获取 tokenizer
    ...
    for batch in trainer.get_eval_dataloader():
        # 将输入数据移到设备上
        inputs = {key: value.to(training_args.device) for key, value in batch.items()}
        count += 1
        print(count)
        print("Start")
        # 获取模型的生成输出
        with torch.no_grad():
            ...
            texts = tokenizer.batch_decode(generate_ids, skip_special_tokens=True)
            # labels = tokenizer.batch_decode(batch['labels'], skip_special_tokens=True)
            print(len(texts))
            # print(len(labels))
            # assert len(texts) == len(labels), "生成文本与真实标签数量不一致"
            for text in texts:
                dic_list.append({"logit_0": text})
